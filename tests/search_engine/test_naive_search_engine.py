...
async def test_naive_search_rerank_and_rerank_top_k(real_kg, kg_fixture_ids):
    ...
    real_kg.index.chunk_vector_db.query = AsyncMock(
        return_value=[
            {"__id__": chunk_ids[0], "distance": 0.2},
            {"__id__": chunk_ids[1], "distance": 0.8},
            {"__id__": "chunk-missing", "distance": 0.5},
        ]
    )
    ...
    engine = NaiveSearchEngine(client=client, knowledge_graph=real_kg, embedder=object(), reranker=reranker)
...
async def test_naive_query_uses_llm_response(monkeypatch):
    ...
    engine = NaiveSearchEngine(client=client, knowledge_graph=kg, embedder=object())
